...
public class Scheduler : ITimeTickReceiver
{
    ...
    private int processEncours = 3;
    //static private long no_of_processes;
    ...
    public void schedule()
    {
        lock (this)
        {
           ...
           // System.Console.Out.WriteLine(removedProcess.TurnAroundTime);
            if (removedProcess != null)
            {
                ...
                {
                    removedProcess.WaitingTime = 2000 * (processEncours-1);
                ...
                {
                    processEncours -=1;
                    Afficher((int)removedProcess.StartTime, (int)removedProcess.TurnAroundTime, (int)removedProcess.WaitingTime, (int)0, (int)0, (int)0);
                    System.Console.Out.WriteLine(clock + "\n\n\tTemps total :  " + removedProcess.TurnAroundTime + " pour le processus " + removedProcess.Name + "\n\n");

                    System.Console.Out.WriteLine(clock + "\tFINISHED: " + removedProcess.Name);
                    System.Console.Out.WriteLine(clock + "\tTemps de fin d'exécution " + removedProcess.Name + ": "+ clock);
                    System.Console.Out.WriteLine(clock + "\tTurnAroundTime " + removedProcess.Name + ": " + removedProcess.TurnAroundTime);
                }
            }
            ...
            if (current != null)
            {
                ...
                if (!current.FirstTimeAdded)
                {
                    current.FirstTimeAdded = true;
                    // System.Console.Out.WriteLine("\n\tPremier ajout de " + current.Name + " dans le processeur au temps : "+clock+"\n\n");
           
                    current.StartTime = clock;
                }
                System.Console.Out.WriteLine(clock + "\tputting on CPU " + current.Name);
                this.currentCPU.SetProcess(current);
                //System.Console.Out.WriteLine(current.TurnAroundTime);
            }
    ...
    public void Afficher(int commencement, int execution, int attente, int executionMoyen, int attenteMoyen, int debit)
    {
        System.Console.Out.WriteLine("\t \n\n");
        System.Console.Out.WriteLine("\t A commencé à : " + commencement);
        System.Console.Out.WriteLine("\t Temps d'exécution : " + execution);
        System.Console.Out.WriteLine("\t Temps d'attente : " + attente);
        System.Console.Out.WriteLine("\t Temps d'exécution moyen : " + executionMoyen);
        System.Console.Out.WriteLine("\t Temps d'attente moyen : " + attenteMoyen);
        System.Console.Out.WriteLine("\t Debit : " + debit);
        System.Console.Out.WriteLine("\t \n\n");
    }
}